...
func GetDayHours(targetWeekday int)[24][4][2]int32 {
	...
	for hour := 0; hour < 24; hour++ {
		hourStart := currentTime.Truncate(time.Hour).Add(time.Duration(hour) * time.Hour)
		hourEnd := hourStart.Add(time.Hour)

		// Populate the result with the corresponding hours of the last five occurrences of the target weekday
		for i := 0; i < 5; i++ {
			weekdayHourStart := hourStart.AddDate(0, 0, -7*i)
			weekdayHourEnd := hourEnd.AddDate(0, 0, -7*i)
