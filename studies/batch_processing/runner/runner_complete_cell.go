	...
	for dayIdx, processedThingsByRoutine := range processedThingsByRoutines {
		for thingName, thing := range processedThingsByRoutine {
			...
			if _, ok := processedThings[name]; !ok {
				...
				processedThings[name].MedianGreenLengthsContinuous[dayIdx] = thing.MedianGreenLengthsContinuous[dayIdx]
				processedThings[name].GreenPhaseCount[dayIdx] = thing.GreenPhaseCount[dayIdx]
			}
		}
	}
